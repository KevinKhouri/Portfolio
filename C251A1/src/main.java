...
public class main {     
    	...
    	/*int[][] tests = {{}, {0}, {1}, {0,1,2}, {0, 2, 4, 6, 8, 10, 12}, {0, 1, 3, 5, 7, 9}, {0, 1, 2, 0, 8, 0, 16, 32,0, 64}, {0, 64, 64, 64, 2, 3, 5, 6, 9, 2000, 102930, 17, 21, 23}};
    	
    	System.out.println("Chaining insert tests(assuming chain is correct):");
    	for (int[] test : tests) {
    		chainingTest(test,5, 0, 21);
    	}
    	
    	System.out.println("Open-Addressing insert tests(assuming probe is correct):");
    	for (int[] test : tests) {
    		openAddTests(test, 5, 0, 21);
    	}
    	System.out.println("Automated tests for chaining: ");
    	for (int i = 0; i < 5; i++) {
    		double n = Chaining.generateRandom(0, 20, i);
    		int w = Chaining.generateRandom(1,  9,  i);
    		double m = Chaining.power2((int) (w-1)/2 + 1);
    		System.out.println("Load Factor: " + n/m);
    		int[] ints = new int[(int) n];
    		for (int j = 0; j < n; j++) {
    			ints[j] = Chaining.generateRandom(0, 20, j);
    		}
    		chainingTest(ints, w, 0, -1);
    	}
    	System.out.println();
    	
    	System.out.println("Automated tests for probing: ");
    	for (int i = 0; i < 5; i++) {
    		double n = Open_Addressing.generateRandom(0, 20, i);
    		int w = Open_Addressing.generateRandom(1,  9,  i);
    		double m = Open_Addressing.power2((int) (w-1)/2 +1);
    		System.out.println("Load Factor: " + n/m);
    		int[] ints = new int[(int) n];
    		for (int j = 0; j < n; j++) {
    			ints[j] = Open_Addressing.generateRandom(0, 20, j);
    		}
    		openAddTests(ints, w, 0, -1);
    	}
    	*/
    }
    /*
    
    public static void chainingTest(int[] inputs, int w, int seed, int A) {
    	Chaining temp = new Chaining(w, seed, A);
		System.out.println(temp.insertKeyArray(inputs) + " Total Collisions.");
		System.out.println(temp.Table);
		System.out.println();
    }
    
    public static void openAddTests(int[] inputs, int w, int seed, int A) {
    	Open_Addressing temp = new Open_Addressing(w, seed, A);
    	System.out.println(temp.insertKeyArray(inputs) + " Total Collisions.");
    	System.out.print("[");
    	for (int i = 0; i <temp.Table.length - 1; i++) {
    		System.out.print(temp.Table[i] + ", ");
    	}
    	System.out.print(temp.Table[temp.Table.length - 1]);
    	System.out.println("]");
    	for(int k = 1; k <= 2; k++) {
    		System.out.println("Removing Key Collisions: " + temp.removeKey(0));
